...
namespace Namotion.Storage.Json
{
    ...
    public class JsonObjectStorage<T> : IObjectStorage<T>
    {
        ...
        {
            return await _blobContainer.ReadAsJsonAsync<T>(id, cancellationToken).ConfigureAwait(false);
        }

        /// <inheritdoc/>
        public async Task WriteAsync(string id, T value, CancellationToken cancellationToken = default)
        {
            await _blobContainer.WriteAsJsonAsync(id, value, cancellationToken).ConfigureAwait(false);
        }
